...
Type objective_function<Type>::operator() ()
{
  ...
  //nll = sum across vector of predictions, evaluate difference between true value and expectation
  //given variance/error distribution, which in this case is normally distributed.
  //this is the probability density
  neglogL = -sum(dnorm(y, ypred, exp(logSigma), true));  //objective function value
  
  //SIMULATE SECTION
  //now the simulate function
  SIMULATE {
    y = rnorm(ypred,exp(logSigma)); //generate random numbers with a mean and sd you specify. 
    REPORT(y);
  }
  
  
  Type sigma2 = square(exp(logSigma)); //convert your logsigma back into a variance in norm dist
  ADREPORT(sigma2);
