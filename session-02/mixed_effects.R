### STREAMS EXAMPLE ###
library(nlme)
Streams <- scan('session-02/data/streams.dat',what=list(Stream=0,Density=0),n=3*18,skip=5)
streams.lm1 <- lm(Density~1,data=Streams)
streams.lm2 <- lm(Density~factor(Stream)-1,data=Streams) 
#stream specific intercept
...
compile("session-02/streams.cpp")
dyn.load(dynlib("session-02/streams"))
