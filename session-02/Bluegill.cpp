...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_VECTOR(x);
  DATA_IVECTOR(tag);
  DATA_IVECTOR(lenMark);
  DATA_IVECTOR(lenRecap);
  DATA_IVECTOR(deltaLen);
  DATA_VECTOR(deltaTime);
  int n = lenMark.size(); //declare interger vector, good for indexing, good if used as ints in the code. 
  //
  PARAMETER(VBK_Linf_mu);
  PARAMETER(Sigma_Linf);
  PARAMETER(Sigma_obs);
  PARAMETER_VECTOR(A_i); //random effect for each fish. 
  PARAMETER_VECTOR(Sigma_A); //random effect
  PARAMETER_VECTOR(Mu_A);
  //for each fish - not sure we need these if we have the vectors. 
  PARAMETER(Mu_L1);
  PARAMETER(Mu_L2);
  PARAMETER(Sigma_l1);
  PARAMETER(Sigma_l2);
