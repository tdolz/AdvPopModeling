...
Type objective_function<Type>::operator() ()
{
  ...
  */
  ...
  // Gary
  //nll -= sum(dnorm(A_i,0,sqrt(exp(log_sigma2_A)),true)); and then Age = mu_A+A_i
  
  
  //nll -= sum(dnorm(u_age,log_shape,exp(log_scale),true));       // Assign N(0,1) distribution u 
  nll -= sum(dnorm(age_1,log_mu_a1,exp(log_sigma_a1),true));       // Assign N(0,1) distribution u 
  ...
  for (int i=0;i<n;i++) {
    ...
    f1(i) = 1.0-exp(-exp(log_k)*exp(age_1(i)));
    f2(i) = 1.0-exp(-exp(log_k)*(exp(age_1(i))+dt(i)));
  ...
  Type k = exp(log_k);
  Type mu_age = exp(log_mu_a1);
  Type sd_age = exp(log_sigma_a1);
  REPORT(exp(age_1));
  REPORT(mu1);
  REPORT(mu2);
  ADREPORT(sigma_inf);
