...
// the process errors are annual so you have to sum over all of them (10/31)- one reason why this is wrong. 
for (int i=0; i<uniqueYear;i++) { //looping over unique years.
  ...
  nll-=dnorm(psi_pups,0,tau2); //we may not need these lines we may not need psi at all (10/31)
